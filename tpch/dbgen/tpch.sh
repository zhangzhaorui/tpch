...
eval PGPASSWORD=\$BSI_${GPINST}_PASSWORD
export PGPASSWORD
...
function benchmark_run() {
	...
	# store the settings
	psql -h $HOST -U $USER $DBNAME -c "select name,setting from pg_settings" > $RESULTS/settings.log 2> $RESULTS/settings.err

	print_log "preparing TPC-H database"

	# create database, populate it with data and set up foreign keys
	# psql -h aa tpch < dss/tpch-create.sql > $RESULTS/create.log 2> $RESULTS/create.err
	...
	psql -h $HOST -U $USER $DBNAME -c "analyze" > $RESULTS/analyze.log 2> $RESULTS/analyze.err
			...
			do
				...
				if [ -d "/proc/$!" ]; then
					...
					if [ $i -eq $DSS_TIMEOUT ]; then
						...
						psql -h $HOST $DBNAME -c "SELECT pg_terminate_backend(procpid) FROM pg_stat_activity WHERE datname = 'tpch'" >> $RESULTS/queries.err 2>&1;

						# time to do a cleanup
						sleep 10;

						# just check how many backends are there (should be 0)
						psql -h  $HOST $DBNAME -c "SELECT COUNT(*) AS tpch_backends FROM pg_stat_activity WHERE datname = 'tpch'" >> $RESULTS/queries.err 2>&1;
